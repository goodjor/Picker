...
{
	TxHeader.StdId = 0;
	TxHeader.ExtId = 0x19C2D2D6;
	TxHeader.RTR = CAN_RTR_DATA;
	TxHeader.IDE = CAN_ID_EXT;
	TxHeader.DLC = 8;
	
	TxData[0] = (0x01 & shift_geer.data_collect.Neutral) | 
...
//												printf("四驱比例系数：%d\r\n",shift_geer.Proportion_Value);
			...
			break;
			//四驱模式
			case 0x19C2D6D2:	shift_geer.Proportion_Value = Rx_Data[0];
												shift_geer.Contol_Pressure = (Rx_Data[2] | (Rx_Data[3]<<8) );
												shift_geer.Write_EEPROM_Flag = 1;
//												printf("四驱比例系数：%d\r\n",shift_geer.Proportion_Value);
...
uint8_t CAN1_SetFilter(uint8_t num, uint32_t Id, uint32_t Mask)
{
  CAN_FilterTypeDef CAN1_FilerConf;

  CAN1_FilerConf.FilterIdHigh = (Id << 3) >> 16; //32?ID
  CAN1_FilerConf.FilterIdLow = (Id << 3) & 0xffff;
  CAN1_FilerConf.FilterMaskIdHigh = (Mask << 3) >> 16; //32
  CAN1_FilerConf.FilterMaskIdLow = (Mask << 3) & 0xffff;
  CAN1_FilerConf.FilterFIFOAssignment = CAN_FILTER_FIFO0; //???0???FIFO0
  CAN1_FilerConf.FilterBank = num;                        //???0
...
//	CAN_FilterTypeDef sFilterConfig;
	...
	CAN1_SetFilter(0, 0x19C1D6D2, 0x01FFFFFF);
